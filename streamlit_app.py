...
if uploaded_file is not None:
    ...
    if 'region' in df.columns and 'site_name' in df.columns and 'temp1' in df.columns:
        ...
        # 🎯 그래프 추가: site_name별 시계열 고온 데이터를 시각화
        st.write("📊 site_name 별 high temp 시계열 그래프 :")
        
        # site_name을 축약하여 표시 ('xx-' 까지만)
        df['short_site_name'] = df['site_name'].apply(lambda x: x.split('-')[0] + '-')

        # 클릭 시 전체 이름이 표시되는 HTML 링크 생성
        for index, row in report_df.iterrows():
            full_name = row['site_name']
            short_name = row['site_name'].split('-')[0] + '-'
            link = f'<a href="#" title="{full_name}">{short_name}</a>'
            st.markdown(link, unsafe_allow_html=True)

        # 시계열 그래프 생성 (site_name별)
        if 'dt' in df.columns and 'temp1' in df.columns:
            fig, ax = plt.subplots()
            for site in report_df['site_name'].unique():
                site_df = df[df['site_name'] == site]
                ax.plot(site_df['dt'], site_df['temp1'], label=site.split('-')[0] + '-')
            ax.set_title('site_name 별 60˚C 이상 고온 SFP 시계열')
            ax.set_xlabel('날짜')
            ax.set_ylabel('온도')
